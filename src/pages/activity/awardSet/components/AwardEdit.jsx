...
function AwardSetPage({ dispatch, awardEdit, form }){
    ...
    function normFileImage(e, size) {
        ...
        if (file) {
            ...
            if (!!size && file.size > size * 1024 * 1024) {   // 若文件大小存在上限 且文件大小超过上限则中止
	...
	function Submit(){
		validateFieldsAndScroll((err, values) => {
			if( !!err ) return;
			if (values.photo.length > 0) {   // 处理图片
                values.photo = !!values.photo[0] && values.photo[0].url ? values.photo[0].url : values.photo[0].response.data[0];
            }
	...
	return (
		<Modal
			...
			footer={[
                ...
                <Button key='submit' type='primary' onClick={Submit} loading={loading} disabled={loading}>保存</Button>
            ]}
		>
				...
				<Form className='modal_form_wrap' >
						...
						{ getFieldDecorator('type', {
							...
							>
								{ awardTypeSels.map((item, index) => {
									return(<Option key={'award_type_' + item.key} value={item.key} >{item.label}</Option>)
								})}
							</Select>
						)}
					</FormItem>
					...
					>
						{ getFieldDecorator('prizeCnt', {
							...
							<InputNumber min={0} max={99999999} style={{width : 140}} placeholder='请输入奖品数量' />
						)}
					</FormItem>
					<FormItem
						{...formItemLayout}
					...
					>
						{ getFieldDecorator('totalCnt', {
							...
							<InputNumber min={0} max={99999999} style={{width : 140}} placeholder='请输入奖品库存' />
						)}
					</FormItem>
					{ editInfo.id !== 1 &&
						<FormItem
                    ...
                    >
                        {getFieldDecorator('photo', {
                            initialValue : editInfo.photo ? [{ uid : -1, url : editInfo.photo, status : 'done' }]
							: [],
                            valuePropName : 'fileList',
                            normalize : (e) => normFileImage(e, 0.5),
                            rules : [
                                { type: 'array', required : true, message : '请上传奖品图'  }
                            ]
                        })(
                            <Upload
                                listType="picture-card"
                                action={`${window.BASE_URL}/file/upload`}
                            >
