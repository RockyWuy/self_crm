...
class CommonSearch extends React.Component {
	...
	ClickToSearch(){    //判断是否 可以搜索
		...
		validateFieldsAndScroll(( err, values ) => {
			...
			for( let [key, value] of Object.entries(values) ){
				...
				if( value !== undefined && value !== '' ){ //去除 为undefined或''的key
					query[ key ] = value;
					isSearch = true;
				}
			}
	...
	//创建Select控件
	createSelect( item ) {
		if( !item.options ){ //options是否传入
			throw new Error('select options undefined');
		}
		if( !window._COM_FUNC.isType('Array')(item.options) ){ //options是否为数组
				...
				{ getFieldDecorator( item.key, {
					...
					>
						{ item.options.map((option, index) => {
							return (
								<Option key={item.key + option.key} value={option[opt_key]} >{option[opt_label]}</Option>
							)
						})}
					</Select>
				)}
