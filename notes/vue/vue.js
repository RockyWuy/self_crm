...
function trigger(target, type, key, newValue, oldValue, oldTarget) {
  ...
  if (type === 'clear' /* CLEAR */) {
    ...
    const isAddOrDelete =
      type === 'add' /* ADD */ || (type === 'delete' /* DELETE */ && !isArray(target));
...
function createGetter(isReadonly = false, shallow = false) {
  return function get(target, key, receiver) {
    if (key === '__v_isReactive' /* IS_REACTIVE */) {
      ...
      key === '__v_raw' /* RAW */ &&
      receiver ===
        (isReadonly ? target['__v_readonly' /* READONLY */] : target['__v_reactive' /* REACTIVE */])
    ) {
      return target;
    }
...
const canObserve = value => {
  return (
    !value['__v_skip' /* SKIP */] && isObservableType(toRawType(value)) && !Object.isFrozen(value)
  );
};
...
function createReactiveObject(target, isReadonly, baseHandlers, collectionHandlers) {
  ...
  const reactiveFlag = isReadonly ? '__v_readonly' /* READONLY */ : '__v_reactive'; /* REACTIVE */
    ...
    set value(newVal) {
      if (hasChanged(toRaw(newVal), rawValue)) {
        ...
        trigger(
          ...
          'set' /* SET */,
          'value',
          process.env.NODE_ENV !== 'production' ? { newValue: newVal } : void 0
        );
      }
    },
  };
...
function triggerRef(ref) {
  trigger(
    ...
    'set' /* SET */,
    'value',
    process.env.NODE_ENV !== 'production' ? { newValue: ref.value } : void 0
  );
}
function unref(ref) {
...
function createSuspenseBoundary(
  ...
  const getCurrentTree = () =>
    suspense.isResolved || suspense.isHydrating ? suspense.subTree : suspense.fallbackTree;
  ...
  process(n1, n2, container, anchor, parentComponent, parentSuspense, isSVG, optimized, internals) {
    ...
    if (n1 == null) {
      ...
      const placeholder = (n2.el =
        process.env.NODE_ENV !== 'production' ? createComment('teleport start') : createText(''));
      const mainAnchor = (n2.anchor =
        process.env.NODE_ENV !== 'production' ? createComment('teleport end') : createText(''));
...
const createVNode =
  process.env.NODE_ENV !== 'production' ? createVNodeWithArgsTransform : _createVNode;
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const unmountComponent = (instance, parentSuspense, doRemove) => {
    ...
    // deactivated hook
    if (da && !isDeactivated && instance.vnode.shapeFlag & 256 /* COMPONENT_SHOULD_KEEP_ALIVE */) {
